...
class Merlin (PyTango.Device_4Impl):
    ...
    def read_framesPerTrigger(self, attr):
        attr.set_value(_MerlinCamera.getFramesPerTrigger())

    @Core.DEB_MEMBER_FUNCT
    def write_framesPerTrigger(self, attr):
        data = attr.get_write_value()
        _MerlinCamera.setFramesPerTrigger(data)
...
def get_control(HostName="", CmdPort=6431, DataPort=6432, ImageWidth=512,
                ImageHeight=512, Chips=4, Simulate=False, **keys):
